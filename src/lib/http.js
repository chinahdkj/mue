...
if (token) sessionStorage.setItem('authortoken', token)
let host = decodeURIComponent(GetQueryString('host') || GetQueryString('host', 'hash'))
if (host && host.indexOf('http://') > -1) sessionStorage.setItem('host', host)
let APP = GetQueryString('app') || GetQueryString('app', 'hash')
if (APP) sessionStorage.setItem('authorapp', APP)

// 测试token
axios.defaults.headers.common['Authorization'] = process.env.NODE_ENV === 'production' ? sessionStorage.getItem('authortoken') : 
...
axios.defaults.headers.common['APP'] = process.env.NODE_ENV === 'production' ? sessionStorage.getItem('authorapp') : 
...
export default {
  post(url, data) {
    return axios({
      method: 'post',
      baseURL: process.env.NODE_ENV === 'production' ? (host && host.indexOf('http://') > -1? host:sessionStorage.getItem('host')): '/list',
      url,
      data: data,
      timeout: 600000
    }).then(
